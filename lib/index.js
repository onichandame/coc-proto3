...
var getWordAtPosition = (doc, pos) => {
  ...
  const firstHalfWords = doc.getText(import_coc3.Range.create(import_coc3.Position.create(pos.line, 0), pos)).split(wordBoundary);
  const secondHalfWords = doc.getText(import_coc3.Range.create(pos, import_coc3.Position.create(pos.line, Number.MAX_VALUE))).split(wordBoundary);
  ...
  return doc.getText(import_coc3.Range.create(pos.line, pos.character - firstHalfWordLength, pos.line, pos.character + secondHalfWordLength));
};
...
(function(Proto3Import2) {
  ...
  Proto3Import2.getImportedFilePathsOnDocument = (document) => {
    ...
    while (importStatement = Proto3Import2.importStatementRegex.exec(fullDocument)) {
      ...
      const searchPath = import_path.default.join(import_coc4.default.workspace.root, "**", protoFileName);
...
var Proto3DefinitionProvider = class {
  constructor() {
    this.provideDefinition = async (document, position) => {
      ...
      const lineText = document.getText(import_coc5.default.Range.create(position.line, 0, position.line, Number.MAX_VALUE));
      ...
      if (matchedGroups && matchedGroups.length === 5) {
        ...
        import_coc5.default.window.showErrorMessage(`Could not find ${targetDefinition} definition.`);
      }
  ...
  async findEnumOrMessageDefinition(document, target) {
    ...
    for (const file of files) {
      const data = import_fs.default.readFileSync(file.toString());
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (messageDefinitionRegexMatch && messageDefinitionRegexMatch.length) {
          const uri = import_coc5.default.Uri.file(file.toString());
          const range = this.getTargetLocationInline(lineIndex, line, target, messageDefinitionRegexMatch);
          return import_coc5.default.Location.create(uri.toString(), range);
        }
      }
    }
  }
  async findImportDefinition(importFileName) {
    const files = await (0, import_fast_glob.default)(import_path2.default.join(import_coc5.default.workspace.root, "**", importFileName));
    const importPath = files[0].toString();
    const uri = import_coc5.default.Uri.file(importPath);
    const definitionStartPosition = import_coc5.default.Position.create(0, 0);
    const definitionEndPosition = import_coc5.default.Position.create(0, 0);
    const range = import_coc5.default.Range.create(definitionStartPosition, definitionEndPosition);
    return import_coc5.default.Location.create(uri.toString(), range);
  }
  getTargetLocationInline(lineIndex, line, target, definitionRegexMatch) {
    ...
    const definitionStartPosition = import_coc5.default.Position.create(lineIndex, index);
    const definitionEndPosition = import_coc5.default.Position.create(lineIndex, index + target.length);
    return import_coc5.default.Range.create(definitionStartPosition, definitionEndPosition);
  }
};

// src/index.ts
async function activate(context) {
  context.subscriptions.push(import_coc6.languages.registerCompletionItemProvider(`proto3`, `proto3`, `proto`, new Proto3CompletionItemProvider(), ["."]), import_coc6.languages.registerDefinitionProvider([PROTO3_MODE], new Proto3DefinitionProvider()));
  import_coc6.workspace.onDidSaveTextDocument((e) => {
    if (e.languageId === `proto`) {
      const folder = import_coc6.workspace.getWorkspaceFolder(e.uri);
      import_coc6.window.showMessage((folder == null ? void 0 : folder.uri) || ``);
    }
  });
}
